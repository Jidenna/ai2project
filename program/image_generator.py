...
def create_id(values={}): 
    try:         
        ...
        precinct_num = '0121B'
        VIN = "1920-"+precinct_num.upper()+"L3189"+first_name[0]+middle_name[0]+last_name[0]+"10000"
        """

        muni_city = values["municipality or city where you live"]
        province = values["province where you live"]
        #VIN = "0809-0121B-L3189JTD10000"
        last_name = values["last name"]
        first_name = values["first name"]
        middle_name = values["middle name"]
        dob = values["birthdate"]
        civil_stat = values["civil status"]
        citizenship = values["citizenship"]
        address1 = values["street name where you live"] + ", " + values["barangay where you live"]
        address2 = values["municipality or city where you live"]
        address3 = values["province where you live"]
        precinct_num = values["precinct number"]
        precinct_num = precinct_num.replace(" ", "")
        VIN = "1920-"+precinct_num.upper()+"-L3189"+first_name[0]+middle_name[0]+last_name[0]+"10000"
        VIN = VIN.upper()
        ...
        name = last_name.replace(" ", "")+first_name.replace(" ", "")+middle_name.replace(" ", "")
        gen_qr_vin(VIN)
        ...
        img_user = Image.open(path+"\\default\\photos\\no_image.png")  
        ...
        draw_img.text(xy=(2300, 1300), text= dob.title(), fill =(0,0,0), font = font_type_content)
        draw_img.text(xy=(2300, 1450), text= civil_stat.title(), fill =(0,0,0), font = font_type_content)
        draw_img.text(xy=(2300, 1600), text= citizenship.title(), fill =(0,0,0), font = font_type_content)
        ...
        draw_img.text(xy=(2300, 2350), text= precinct_num.upper(), fill =(0,0,0), font = font_type_content)
        
        # insert user signature into the template
        #img_user_sign = Image.open(path+"\\data\\signs\\"+VIN+".png")  
        img_user_sign = Image.open(path+"\\default\\signs\\no_sign.png")  
